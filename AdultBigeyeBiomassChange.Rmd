...
Bigeye_Min_BiomassPercentChange <- matrix(NA, 
                                          nrow = (dim(lat_centers)[1] * dim(lon_centers)[1]),
                                          ncol = 4)
Bigeye_Min_BiomassPercentChange <- data.frame(Bigeye_Min_BiomassPercentChange)
colnames(Bigeye_Min_BiomassPercentChange) <- c("LatCenter", "LonCenter", "RCP4.5", "RCP8.5")

Bigeye_Mean_BiomassPercentChange <- matrix(NA, 
                                           nrow = (dim(lat_centers)[1] * dim(lon_centers)[1]),
                                           ncol = 4)
Bigeye_Mean_BiomassPercentChange <- data.frame(Bigeye_Mean_BiomassPercentChange)
colnames(Bigeye_Mean_BiomassPercentChange) <- c("LatCenter", "LonCenter", "RCP4.5", "RCP8.5")

Bigeye_Max_BiomassPercentChange <- matrix(NA, 
                                          nrow = (dim(lat_centers)[1] * dim(lon_centers)[1]),
                                          ncol = 4)
Bigeye_Max_BiomassPercentChange <- data.frame(Bigeye_Max_BiomassPercentChange)
colnames(Bigeye_Max_BiomassPercentChange) <- c("LatCenter", "LonCenter", "RCP4.5", "RCP8.5")

Bigeye_SignAgreement_BiomassPercentChange <- matrix(NA, 
                                                    nrow = (dim(lat_centers)[1] * dim(lon_centers)[1]),
                                                    ncol = 4)
Bigeye_SignAgreement_BiomassPercentChange <- data.frame(Bigeye_SignAgreement_BiomassPercentChange)
colnames(Bigeye_SignAgreement_BiomassPercentChange) <- c("LatCenter", "LonCenter", "RCP4.5", "RCP8.5")
```
...
t_future_idx_45 <- which(gfdl_45_bet_adult$t >= 2044 & gfdl_45_bet_adult$t < 2054)
t_future_idx_85 <- which(gfdl_85_bet_adult$t >= 2044 & gfdl_85_bet_adult$t < 2054)

# RCP4.5
# empty grids to fill
baseline_biomass_45 <- array(NA, dim = c(length(gfdl_45_bet_adult$x),
                                         length(gfdl_45_bet_adult$y),
                                         4),
                             dimnames = list("x" = gfdl_45_bet_adult$x,
                                             "y" = gfdl_45_bet_adult$y,
                                             "m" = c("GFDL", "IPSL", "MIROC", "MPI")))
...
delta_biomass_45 <- array(NA, dim = c(length(t_future_idx_45),
                                      ...
                                      length(gfdl_45_bet_adult$y),
                                      4),
                          dimnames = list("t" = gfdl_45_bet_adult$t[t_future_idx_45],
                                          "x" = gfdl_45_bet_adult$x,
                                          "y" = gfdl_45_bet_adult$y,
                                          "m" = c("GFDL", "IPSL", "MIROC", "MPI")))
...
# RCP8.5
# empty grids to fill
baseline_biomass_85 <- array(NA, dim = c(length(gfdl_85_bet_adult$x),
                                         length(gfdl_85_bet_adult$y),
                                         4),
                             dimnames = list("x" = gfdl_85_bet_adult$x,
                                             "y" = gfdl_85_bet_adult$y,
                                             "m" = c("GFDL", "IPSL", "MIROC", "MPI")))
...
delta_biomass_85 <- array(NA, dim = c(length(t_future_idx_85),
                                      ...
                                      length(gfdl_85_bet_adult$y),
                                      4),
                          dimnames = list("t" = gfdl_85_bet_adult$t[t_future_idx_85],
                                          "x" = gfdl_85_bet_adult$x,
                                          "y" = gfdl_85_bet_adult$y,
                                          "m" = c("GFDL", "IPSL", "MIROC", "MPI")))
```
...
# Time steps of interest
# the RCP4.5 and RCP8.5 grids are different sizes
t_baseline_idx_45 <- which(gfdl_45_bet_adult$t >= 2011 & gfdl_45_bet_adult$t < 2021)
t_baseline_idx_85 <- which(gfdl_85_bet_adult$t >= 2011 & gfdl_85_bet_adult$t < 2021)

# Loop through each model and grid cell, averaging across all time steps
# RCP4.5
for (cm in seq(1, 4, 1)) {
  for (r in seq(1, length(gfdl_45_bet_adult$y), 1)) {
    for (c in seq(1, length(gfdl_45_bet_adult$x))) {
...
# RCP8.5
for (cm in seq(1, 4, 1)) {
  for (r in seq(1, length(gfdl_85_bet_adult$y), 1)) {
    for (c in seq(1, length(gfdl_85_bet_adult$x))) {
      # Select period of interest
...
for (t in seq(1, length(t_future_idx_45), 1)) {
  for (cm in seq(1, 4, 1)) {
    for (r in seq(1, length(gfdl_45_bet_adult$y), 1)) {
      for (c in seq(1, length(gfdl_45_bet_adult$x), 1)) {
...
for (t in seq(1, length(t_future_idx_85), 1)) {
  for (cm in seq(1, 4, 1)) {
    for (r in seq(1, length(gfdl_85_bet_adult$y), 1)) {
      for (c in seq(1, length(gfdl_85_bet_adult$x), 1)) {
...
for (r in seq(1, dim(lat_centers)[1], 1)) {
  for (c in seq(1, dim(lon_centers)[1], 1)) {
    ...
    Bigeye_Min_BiomassPercentChange$RCP4.5[bpc_counter] <- signif(min(delta_hold_45, na.rm = TRUE), 5)
    Bigeye_Mean_BiomassPercentChange$RCP4.5[bpc_counter] <- signif(mean(delta_hold_45, na.rm = TRUE), 5)
    Bigeye_Max_BiomassPercentChange$RCP4.5[bpc_counter] <- signif(max(delta_hold_45, na.rm = TRUE), 5)
    ...
    if (sum(is.na(delta_hold_45)) + sum(delta_hold_45 > 0, na.rm = TRUE) == 1920 | 
      ...
      Bigeye_SignAgreement_BiomassPercentChange$RCP4.5[bpc_counter] <- 1
    }
    ...
    Bigeye_Min_BiomassPercentChange$RCP8.5[bpc_counter] <- signif(min(delta_hold_85, na.rm = TRUE), 5)
    Bigeye_Mean_BiomassPercentChange$RCP8.5[bpc_counter] <- signif(mean(delta_hold_85, na.rm = TRUE), 5)
    Bigeye_Max_BiomassPercentChange$RCP8.5[bpc_counter] <- signif(max(delta_hold_85, na.rm = TRUE), 5)
    
    # Looking across 120 time steps, 2 zonal grid cells, 2 meridional grid cells, 4 earth system models
    # 120 x 2 x 2 x 4 = 1920
    if (sum(is.na(delta_hold_85)) + sum(delta_hold_85 > 0, na.rm = TRUE) == 1920 | 
        sum(is.na(delta_hold_85)) + sum(delta_hold_85 > 0, na.rm = TRUE) == 0) {
      Bigeye_SignAgreement_BiomassPercentChange$RCP8.5[bpc_counter] <- 1
    } 
    
    # The right-hand side of lat & lon fills are being treated as 1x1 tibbles
    # so turning them into numbers
    Bigeye_Min_BiomassPercentChange$LatCenter[bpc_counter] <- as.numeric(lat_centers[r,1])
    Bigeye_Mean_BiomassPercentChange$LatCenter[bpc_counter] <- as.numeric(lat_centers[r,1])
    Bigeye_Max_BiomassPercentChange$LatCenter[bpc_counter] <- as.numeric(lat_centers[r,1])
    Bigeye_SignAgreement_BiomassPercentChange$LatCenter[bpc_counter] <- as.numeric(lat_centers[r,1])
    
    Bigeye_Min_BiomassPercentChange$LonCenter[bpc_counter] <- as.numeric(lon_centers[c,1])
    Bigeye_Mean_BiomassPercentChange$LonCenter[bpc_counter] <- as.numeric(lon_centers[c,1])
    Bigeye_Max_BiomassPercentChange$LonCenter[bpc_counter] <- as.numeric(lon_centers[c,1])
    Bigeye_SignAgreement_BiomassPercentChange$LonCenter[bpc_counter] <- as.numeric(lon_centers[c,1])
...
```{r}
# Save output
write_csv(Bigeye_Min_BiomassPercentChange, "Bigeye_Min_BiomassPercentChange.csv")
write_csv(Bigeye_Mean_BiomassPercentChange, "Bigeye_Mean_BiomassPercentChange.csv")
write_csv(Bigeye_Max_BiomassPercentChange, "Bigeye_Max_BiomassPercentChange.csv")
write_csv(Bigeye_SignAgreement_BiomassPercentChange, "Bigeye_SignAgreement_BiomassPercentChange.csv")
```
